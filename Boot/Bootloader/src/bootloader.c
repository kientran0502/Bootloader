...
//                    NVIC_SystemReset();
                    uint32_t checksum = 0;
                    uint8_t *p = (uint8_t*)0x0040C000;
                    for (uint32_t i = 0; i < 201098; i++) 
                    {
                        checksum += p[i];
                    }
                    uint8_t str2[7] ={0}; 
                    sprintf(str2, "%d", checksum, 7);
                    BlTransport_Write(str2,7);
...
//    pmc_disable_periph_clk(ID_UART2);
...
//    UART2_Write(str, 64);
    delay(32);  // must to delay
    
	/** Disable interrupts */
	__disable_irq();
    
        /* Deinit peripherals if needed */
    SysTick->CTRL = 0;
    ...
    SCB_CleanInvalidateDCache(); // CRITICAL FOR CORTEX-M7
