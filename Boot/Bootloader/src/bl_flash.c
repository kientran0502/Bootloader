...
BlStatus_t BlFlash_Write(uint32_t addr, uint8_t *data, uint32_t len)   // ?amt b?o chia h?t 16
{
    ...
    while (len > 0) 
    {
        ...
        uint32_t offset = addr & 0xF;        // V? tr� l?ch trong QuadWord
        ...
        // 2. S?A: Ch? ?� ph?n d? li?u m?i nh?n t? UART v�o ?�ng v? tr�
        memcpy(&page_buffer[offset], data, chunk);

        // 3. GHI: Ghi c? c?m 16 byte ?� ho�n ch?nh v�o Flash
...
//BlStatus_t BlFlash_Write(uint32_t addr, uint8_t *data, uint32_t len)
//{
//    // 1. T?t Cache to�n c?c (N?u b?n kh�ng t?t ? main th� ?? ? ?�y)
//    SCB_DisableDCache(); 
//    SCB_DisableICache();
//    __DSB();
//    __ISB();
...
//
//        // 2. READ-MODIFY-WRITE (L�c n�y memcpy s? ??c TR?C TI?P t? Flash)
...
//        while (EFC_IsBusy());
//        
//        // ??c thanh ghi ?? �p Bus ??ng b? d? li?u th?c t? v�o � nh?
...
//    }
    ...
    // 5. B?t l?i Cache sau khi n?p xong ho�n to�n (N?u c?n thi?t)
    // SCB_EnableICache();
    // SCB_EnableDCache();
